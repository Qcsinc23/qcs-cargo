...
// shipRequestEstimate returns the persisted pricing snapshot from the
// last customs submission. DEF-007 (backlog) note: this endpoint is
// intentionally a snapshot, not a live recompute — pricing inputs (weight,
// dimensions, destination tier) only change inside POST /customs, and the
// snapshot is the canonical input to /pay. Callers that need a fresh
// quote should resubmit customs. The response includes a
// `snapshot_at` so the UI can label the value with its age.
func shipRequestEstimate(c *fiber.Ctx) error {
	...
	return c.JSON(fiber.Map{
		"data": fiber.Map{
			...
			"total":        sr.Total,
			"snapshot_at":  sr.UpdatedAt,
		},
	})
}

// shipRequestPay, isReusablePaymentIntent, and shipRequestReconcile have
// moved to ship_requests_payment.go (Phase 3.3 / QAL-001 split).
