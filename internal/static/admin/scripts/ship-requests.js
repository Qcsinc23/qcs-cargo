...
// the CSP can drop 'unsafe-inline' (Phase 3.1).
    ...
    (function() {
      ...
      if (!token) { window.location.href = '/login?redirect=' + encodeURIComponent('/admin/ship-requests'); return; }
      ...
      function adminSidebar(active) {
        ...
        return '<div class="flex min-h-screen"><aside class="w-64 bg-[#0F172A] text-white py-6 px-4">' +
          ...
          '<nav class="space-y-1"><a href="/admin" class="block py-2 px-3 rounded-lg hover:bg-[#1E293B]">Dashboard</a>' +
          '<a href="/admin/ship-requests" class="block py-2 px-3 rounded-lg ' + (active === 'ship-requests' ? 'bg-[#1E293B]' : 'hover:bg-[#1E293B]') + '">Ship Requests</a>' +
          ...
          '<a href="/admin/activity" class="block py-2 px-3 rounded-lg hover:bg-[#1E293B]">Activity</a></nav>' +
      ...
      fetch('/api/v1/me', auth).then(r => { if (!r.ok) { window.location.href = '/login'; return null; } return r.json(); }).then(me => {
        ...
        fetch('/api/v1/admin/ship-requests' + q, auth).then(r => r.ok ? r.json() : null).then(res => {
          const list = (res && res.data) || [];
          let main = '<main class="flex-1 p-8"><nav class="mb-4 text-sm text-slate-600"><a href="/admin" class="text-[#2563EB] hover:underline">Admin</a> / Ship Requests</nav>' +
            '<h1 class="text-3xl font-bold mb-6">Ship Requests</h1>' +
            '<p class="text-slate-600 mb-4">' + list.length + ' request(s)</p>' +
            ...
            '<th class="text-left py-3 px-4 font-medium text-slate-600">Code</th><th class="text-left py-3 px-4 font-medium text-slate-600">User</th><th class="text-left py-3 px-4 font-medium text-slate-600">Status</th>' +
            '<th class="text-left py-3 px-4 font-medium text-slate-600">Created</th></tr></thead><tbody>';
          ...
          list.forEach(sr => {
            main += '<tr class="border-b border-slate-100 hover:bg-slate-50">' +
              '<td class="py-3 px-4 font-mono text-sm">' + esc(sr.confirmation_code || sr.id) + '</td>' +
              '<td class="py-3 px-4 text-sm">' + esc(sr.user_id ? sr.user_id.slice(0, 8) + '…' : '—') + '</td>' +
              '<td class="py-3 px-4"><span class="px-2 py-1 rounded text-sm bg-slate-100">' + esc(sr.status || '—') + '</span></td>' +
              '<td class="py-3 px-4 text-slate-500 text-sm">' + esc(sr.created_at ? sr.created_at.slice(0, 10) : '—') + '</td></tr>';
          });
          main += '</tbody></table></div></main></div>';
          document.getElementById('app').innerHTML = adminSidebar('ship-requests') + main;
