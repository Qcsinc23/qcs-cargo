// Auto-extracted from exceptions.html
...
// the CSP can drop 'unsafe-inline' (Phase 3.1).
    ...
    (function() {
      ...
      if (!token) { window.location.href = '/login?redirect=' + encodeURIComponent('/warehouse/exceptions'); return; }
      const auth = { headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' } };
      fetch('/api/v1/me', auth).then(r => {
        if (!r.ok) { window.location.href = '/login?redirect=' + encodeURIComponent('/warehouse/exceptions'); return null; }
        return r.json();
      }).then(me => {
        if (!me) return;
        ...
        const sidebar = '<aside class="w-64 bg-[#0F172A] text-white py-6 px-4"><a href="/" class="font-bold text-lg block mb-8">QCS Cargo</a><p class="text-amber-400 text-sm font-medium mb-2">Warehouse</p><nav class="space-y-1">' +
          ...
          '<a href="/warehouse/exceptions" class="block py-2 px-3 rounded-lg bg-[#1E293B]">Exceptions</a></nav>' +
        ...
        function load() {
          fetch('/api/v1/warehouse/exceptions', auth).then(r => r.json()).then(j => {
            const list = (j && j.data) || [];
            // Pass 2 audit fix C-1: HTML-escape every server-supplied field
            // (carrier, tracking_number, status are operator/inbound-supplied).
            const esc = window.QCSAdmin.escapeHTML;
            const rows = list.map(e => {
              const resolveBtn = '<button type="button" class="resolve-btn px-3 py-1 rounded bg-emerald-600 text-white text-sm" data-id="' + esc(e.id || '') + '">Resolve</button>';
              return '<tr class="border-b border-slate-200"><td class="py-3 px-4 font-mono text-sm">' + esc(e.id ? e.id.substring(0, 8) + '…' : '—') + '</td><td class="py-3 px-4">' + esc(e.tracking_number || '—') + '</td><td class="py-3 px-4">' + esc(e.carrier || '—') + '</td><td class="py-3 px-4">' + esc(e.status || '—') + '</td><td class="py-3 px-4">' + esc(e.received_at || '—') + '</td><td class="py-3 px-4">' + resolveBtn + '</td></tr>';
            }).join('');
            const main = '<main class="flex-1 p-8"><nav aria-label="Breadcrumb" class="mb-4"><ol class="flex items-center gap-2 text-sm text-slate-600"><li><a href="/warehouse" class="hover:underline">Warehouse</a></li><li aria-current="page" class="font-medium">Exceptions</li></ol></nav><h1 class="text-3xl font-bold mb-6">Exceptions (unmatched packages)</h1><div class="bg-white rounded-xl border border-slate-200 overflow-hidden"><table class="w-full"><thead class="bg-slate-50"><tr><th class="text-left py-3 px-4">ID</th><th class="text-left py-3 px-4">Tracking</th><th class="text-left py-3 px-4">Carrier</th><th class="text-left py-3 px-4">Status</th><th class="text-left py-3 px-4">Received</th><th class="text-left py-3 px-4">Action</th></tr></thead><tbody>' + rows + '</tbody></table></div></main>';
            ...
            document.querySelectorAll('.resolve-btn').forEach(btn => {
              btn.onclick = function() {
                ...
                const action = prompt('Action: match, return, or dispose', 'match');
                fetch('/api/v1/warehouse/exceptions/' + id + '/resolve', { method: 'POST', headers: auth.headers, body: JSON.stringify({ resolution: resolution || undefined, action: action || 'match' }) })
                  .then(r => r.json()).then(j => { if (j.data) load(); else alert((j.error && j.error.message) || 'Failed'); });
              };
            });
          }).catch(() => { document.getElementById('app').innerHTML = '<div class="p-8">Failed to load exceptions.</div>'; });
        }
        load();
      });
    })();
