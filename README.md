...
The deploy path now uses pinned host keys instead of runtime `ssh-keyscan`, deploys from a clean detached git worktree on the server so stray local edits cannot block releases, keeps deployment state and locking under `.deploy/`, verifies both container health and public endpoints, and records the last successful release SHA for best-effort rollback. That rollback does not undo database migrations; use backups if a schema change is not backward compatible.
